...
import { LoginUserWithPasswordRequest, LoginValidationSchema } from "@/types/user.types";
...
export default function LoginForm() {
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			...
			const loginPromise = frontendAxios.post("/auth/login", values);

			toast.promise(loginPromise, {
				loading: "Logging in...",
				success: (response: AxiosResponse<T_SEResponse<LoginUserWithPasswordRequest>>) => {
					...
					if (data.success) {
						...
						setLoading(false);
						// Safely access data.errors[0] with a fallback
						const errorMessage =
							Array.isArray(data.errors) && data.errors.length > 0
								? data.errors[0]
								: "Failed to sign in, Please check the credentials";
	...
	return (
		<form onSubmit={formik.handleSubmit} className="space-y-6">
			...
			<Button
				...
				className="w-full cursor-pointer h-12 flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white"
			>
