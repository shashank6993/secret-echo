...
const ExpandableChat: React.FC<ExpandableChatProps> = ({
	...
	return (
		<div className={cn(`fixed ${chatConfig.positions[position]} z-20`, className)} {...props}>
			<div
				ref={chatRef}
				className={cn(
					"flex flex-col bg-background border sm:rounded-[14px] shadow-md overflow-hidden transition-all duration-250 ease-out sm:absolute sm:w-[90vw] sm:h-[80vh] fixed inset-0 w-full h-full sm:inset-auto",
...
const ExpandableChatToggle: React.FC<ExpandableChatToggleProps> = ({
	...
	<div className="relative">
		<Button
			...
			onClick={toggleChat}
			className={cn(
				"w-14 h-14 rounded-md p-2 bg-[#F2F2F7] z-20 hover:bg-[#F2F2F7] shadow-md flex items-center justify-center hover:shadow-lg hover:shadow-black/30 transition-all duration-300",
				className
			)}
			{...props}
		>
			{isOpen ? <X className="h-7 w-7 text-slate-600" /> : icon || <MessageCircle className="h-6 w-6" />}
		</Button>
	</div>
);
